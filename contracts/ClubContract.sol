...
contract ClubContract is Ownable, Initializable, IClubContract {
    uint256 totalIdeas;
    string public name;
        ...
        initializer
    {
        ...
        (_name, _metadata) = dataDecoder(_data);
        name = _name;
        metadata = _metadata;
    ...
    function dataDecoder(bytes memory data)
        internal
        pure
        returns (string memory _name, string memory _metadata)
    {
        (_name, _metadata) = abi.decode(data, (string, string));
    }
    ...
    // function isMember(address _member) external pure override returns (bool) {
    //     return (_member != address(0));
    // }
    ...
    function _memberWeight(address _member) internal pure returns (uint256) {
        if (_member != address(0)) return 1;
        else return 0;
    }
    ...
    function vote(uint256 _index, uint256 _type)
        external
        override
        initialized
        weightMoreThenZero
